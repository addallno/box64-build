...
X64_ROBUST_LINREST_INSERT = (
    '        // restore-lin：恢复 my274/lin274 伪造时保存的 +0x190 槽位原值\n'
...
X64_ROBUST_MYSCREST_INSERT = (
    '        // restore-mys：恢复 my274 伪造时保存的 +0x190 槽位原值\n'
    '        if(rb_rest.pending && rb_rest.tid == GetTID()) {\n'
    '            if(rb_rest.addr_h) *(uintptr_t*)rb_rest.addr_h = rb_rest.val_h;\n'
    '            if(rb_rest.addr_l) *(uintptr_t*)rb_rest.addr_l = rb_rest.val_l;\n'
    '            rb_rest.pending = 0;\n'
...
X64_ROBUST_MYSC_INSERT = (
    ...
    '                // 断言检查帧比 syscall 入口帧高 0x190（两次实测），补写 +0x190 槽位；\n'
    '                // 该槽位原值是上层帧 flag/canary，先保存，后续同线程 syscall 入口恢复\n'
    '                rb_rest.pending = 1;\n'
    '                rb_rest.tid = GetTID();\n'
